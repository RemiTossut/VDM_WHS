...
target_table ="Spuntini_Ghislenghien_5525976_V0_metadata"

# --- CONFIG ---
landing_root = "/Volumes/d_dnst_000_landing/volumes/sellout_data/5525976___Spuntini_Ghislenghien___V1/"
sharepoint_folder = "Collection sell-out data/5525976 - Spuntini Ghislenghien - V1"

config = {
    "file_pattern": "*.xlsx",           # Only Excel files
    "folder_name_skip": [],        # Skip these folders
    "sheet_name": [],                 # If set, only this sheet is processed
    "sheet_name_skip": ["Source Identification"],      # Skip these sheets when processing all sheets
}
...
def download_and_stage_excel_files():
    ...
    for sp_path in files_to_download:
        try:
            ...
            print(f"sheet names for {sp_path} path: {names} ")

            # --- Determine sheets to process ---
            if config["sheet_name"]:
                sheets_to_process = [s for s in config["sheet_name"] if s in xls.sheet_names]
            else:
                sheets_to_process = [s for s in xls.sheet_names if s not in (config["sheet_name_skip"] or [])]
            print("sheets_to_process: ", sheets_to_process)
